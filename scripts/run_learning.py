...
from wheeledsim_land.networks.cnn_blocks.cnn_blocks import ResnetCNN
...
if __name__ == '__main__':
    ...
    buf = InterventionReplayBuffer(spec, capacity=50000).to('cpu')
    net = ResnetCNN(insize=[3, 64, 64], outsize=steer_n, n_blocks=2, pool=4, mlp_hiddens=[32, ]).to('cpu')
    ...
    print('waiting 1s for topics...', int(1/spec.dt))
    ...
    batch = dict_to(dict_map(batch, lambda x:x.unsqueeze(0)), 'cpu')
    buf.insert(batch)
